...
- **Agent control surface at PIE runtime.** `UnrealBridgeGameplayLibrary` provides aggregated world observation, navigation pathfinding, movement / look / jump operations, and focus-independent gameplay mouse input. Relative motion, left/right/middle button edges and clicks, and wheel input travel through the first local PIE player's GameViewport / PlayerInput / Enhanced Input chain without moving the system cursor or focusing the Editor.
...
flowchart LR
    ...
    subgraph Host["Agent host"]
      ...
      Mani[("bridge_manifest.json<br/>26 libs · 1396 methods")]
    end

    Gen["tools/gen_manifest.py<br/>reflects the live UE API"]
...
bridge.py --project=Key pie-mouse button left click
...
| `UnrealBridgeGameplayLibrary` | PIE-runtime Agent control: aggregated world observation, navigation pathfinding; movement / look / jump / teleport / sticky input; **focus-independent gameplay mouse input** (relative MouseX/Y, left/right/middle press/release/click, wheel, diagnostics and teardown cleanup) routed through the first local player's GameViewport / PlayerInput / Enhanced Input chain without OS cursor or Slate pointer simulation; Enhanced Input runtime injection plus IA / IMC enumeration and IMC mapping authoring (`list_input_actions` / `list_input_mapping_contexts` / `get_input_mapping_context_mappings` / `add_ia_mapping_to_imc` / `remove_ia_mapping_from_imc`); pawn velocity, ability, jump-arc simulation; camera ray, screen ↔ world, NavMesh projection; damage, physics impulse, time dilation, sound, camera shake; debug draw; AI-controller probing |
